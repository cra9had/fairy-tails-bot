...
def get_child_settings_window():
    window = Window(
        ...
        Column(
            ...
            Button(Format("Увлечения{activities}"), id="activities", on_click=switch_to_activities),
...
def get_child_activities_window():
    ...
    window = Window(
        ...
        *[Button(Const(i), id=str(idx), on_click=set_child_activities) for idx, i in enumerate(activities)],
...
def get_profile_window():
    window = Window(Const("Privet"), state=Profile.my_profile)
    return window
